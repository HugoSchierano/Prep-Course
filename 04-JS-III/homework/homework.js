...
function promedioResultadosTest(resultadosTest) {
  ...
  // Tu código:
  promedioResultadosTest = 0;
  suma = 0; {
    for (let i = 0; i < resultadosTest.length; i++) {
      suma = suma + resultadosTest[i];
    } promedioResultadosTest = suma / resultadosTest.length;
...
function numeroMasGrande(numeros) {
  ...
  numeroMasGrande = numeros[0]; {
    for (let i = 1; i < numeros.length; i++) {
      if (numeroMasGrande > numeros[i]) return numeroMasGrande;
      else numeroMasGrande = numeros[i];
    }
...
for ( let i = 0; i < arreglo.length; i ++ ) {
  if ( arreglo[i] > 18 ) cuentoElementos = cuentoElementos + 1 ;
}
...
function empiezaConNueve(n) {
  ...
  if ( primerNumero[0] === '9' ) return true ;
  else return false ;
}
